  ...
  stages {
    ...
    stage('Commit & Push') {
      steps {
        dir("k8s-acrogcd/jenkins-demo") {
          ...
          sh 'git remote set-url origin https://$GITHUB_TOK@github.com/ashwinbittu/k8s-acrogcd'
    ...
    stage('Raise PR') {
      steps {
        ...
        sh 'gh pr create --title "Updated PHP k8s App" --body "Updated deployment specification with a new image version."'
        sh 'echo "Success"'        
      }
    } 
  }
